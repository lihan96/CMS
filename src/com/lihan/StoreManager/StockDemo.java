...
public class StockDemo extends JPanel{
	...
	StockDemo(){
		...
		this.setLayout(null);
		JLabel title = new JLabel("进货单添加");
		title.setBounds(10, 10, 270, 80);
		title.setForeground(new Color(0, 152, 210));
		title.setFont(new Font("新宋体", java.awt.Font.PLAIN, 32));
		this.add(title);

		JLabel[] jLabels = {new JLabel("* 进货编号"), new JLabel("* 供应商编号:"),
				new JLabel("* 商品编号:"), new JLabel("* 进货时间:"), new JLabel("* 数    量:")};
		JLabel info = new JLabel("请输入进货信息：");
		...
		btnAdd.addActionListener(e -> {
			...
			try {
				...
				String sql = "INSERT INTO tb_stock(sto_id,sup_id,pro_id,date,number)" +
